...
const BotIcon = () => (
  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
    <path d="M20 9V7c0-1.1-.9-2-2-2h-3c0-1.66-1.34-3-3-3S9 3.34 9 5H6c-1.1 0-2 .9-2 2v2c-1.66 0-3 1.34-3 3s1.34 3 3 3v4c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-4c1.66 0 3-1.34 3-3s-1.34-3-3-3zm-2 11H6V7h12v13zm-5-8h-2v-2h2v2zm-4 0H7v-2h2v2zm8 0h-2v-2h2v2z" />
  </svg>
);
const NumberIcon = () => (
  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    <aside className="w-[220px] shrink-0 bg-surface border-r border-border flex flex-col sticky top-0 h-screen">
      ...
      <div className="px-5 pt-6 pb-4 border-b border-border">
        ...
        {isDemo && (
          <div
            className="flex items-center gap-1.5 text-[10.5px] font-medium text-brand-amber
...
function NavItem({ label, Icon, active, onClick, badge }) {
  return (
    ...
    >
      ...
      {badge != null && (
        <span
          className="ml-auto text-[10px] font-bold bg-brand-red text-white rounded-full
...
function SubItem({ label, Icon, active, onClick }) {
  return (
    ...
    >
      <span className={`w-3.5 h-3.5 ${active ? "text-accent" : "text-tx-3"}`}>
        <Icon />
      </span>
      {label}
    </button>
