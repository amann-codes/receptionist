...
import {
  getToken, clearToken, isDemoCredentials,
  login, verifyToken,
  fetchStats, fetchCalls, patchStatus, patchNotes, deleteCall,
  fetchAgentConfig, saveAgentConfig,
} from './api'
import { useToast } from './ToastContext'

const PhoneIcon = () => <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 0 0-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>
const MailIcon  = () => <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>
const AlertIcon = () => <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
const WorkIcon  = () => <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M20 7H4c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V9c0-1.1-.9-2-2-2zm-9-2h2v2h-2V5zm9 14H4V9h16v10z"/></svg>
...
export default function App() {
  ...
  // ── Agent save ─────────────────────────────────────────────────────────────
  const handleSaveAgent = useCallback(async (cfg) => {
    if (isDemo) {
      setAgentConfig(cfg)
  ...
  // ── Call actions ───────────────────────────────────────────────────────────
  const handleAction = useCallback(async (callId, status) => {
    setCalls(prev => prev.map(c => c.call_id === callId ? { ...c, status } : c))
  ...
  const handleDelete = useCallback(async (callId) => {
    ...
    if (!isDemo) {
      try {
        ...
        const s = await fetchStats()
        setStats(s)
      } catch { toast('Failed to delete call', 'error') }
    }
  }, [isDemo, toast])
  ...
  const filtered = useMemo(() => {
    ...
    if (['unread', 'actioned', 'read'].includes(filter))
      ...
      list = list.filter(c => c.intent === filter)
    if (search.trim()) {
  ...
  const pageTitle = {
    all: 'All Calls', unread: 'Unread', actioned: 'Actioned',
    urgent: 'Urgent', work: 'Work', personal: 'Personal', spam: 'Spam',
  }[filter] || 'All Calls'
  ...
  if (!authReady) {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <svg className="w-5 h-5 text-tx-3 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <Sidebar
        ...
        onNavigate={(p) => { setPage(p); setSelected(null) }}
        user={user}
        isDemo={isDemo}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <StatusBar connected={connected} isDemo={isDemo} />

        {/* ── Page: Agent ── */}
        {page === 'agent' && agentConfig && (
          <AgentPage agentConfig={agentConfig} onSave={handleSaveAgent} isDemo={isDemo} />
        )}

        {/* ── Page: Number ── */}
        {page === 'number' && (
          <NumberPage user={user} isDemo={isDemo} />
        )}
            ...
            <div className="flex-1 overflow-y-auto px-8 py-7">
              ...
              <div className="grid grid-cols-4 gap-3.5 mb-7">
                {loading && !stats
                  ...
                  : <>
                      <StatCard label="Total calls" value={liveStats.total}           meta="All time"       icon={PhoneIcon} iconClass="bg-accent-dim text-accent" />
                      <StatCard label="Unread"      value={liveStats.unread}          meta="Need attention" icon={MailIcon}  iconClass="bg-brand-amber-dim text-brand-amber" />
                      <StatCard label="Urgent"      value={liveStats.urgent}          meta="High priority"  icon={AlertIcon} iconClass="bg-brand-red-dim text-brand-red" />
                      <StatCard label="Work calls"  value={liveStats.by_intent?.work} meta="Professional"   icon={WorkIcon}  iconClass="bg-brand-green-dim text-brand-green" />
                    </>
                }
              </div>
              ...
              <div className="flex items-center gap-2 mb-4 flex-wrap">
                ...
                <div className="ml-auto relative">
                  <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-tx-3 pointer-events-none"
              ...
              {loading ? (
                <div className="flex flex-col gap-2">
                  {Array.from({ length: 5 }).map((_, i) => <CallCardSkeleton key={i} />)}
                </div>
              ) : filtered.length === 0 ? (
                ...
                <div className="flex flex-col gap-2 animate-fadeIn">
                  {filtered.map(call => (
                    <CallCard key={call.call_id} call={call} onClick={handleOpen} onAction={handleAction} />
                  ))}
                </div>
              )}
            </div>
    ...
    <div className="flex flex-col items-center justify-center py-24 gap-4 text-tx-3 animate-fadeIn">
      ...
      <div className="text-center">
        <p className="text-[14px] font-medium text-tx-2">
          {hasSearch ? `No results for "${search}"` : `No ${filter === 'all' ? '' : filter + ' '}calls`}
        </p>
        <p className="text-[12px] mt-1">
          {hasSearch ? 'Try a different search term'
            : filter !== 'all' ? 'No calls in this category yet'
            : 'Calls will appear here once your agent receives them'}
        </p>
      </div>
      {(hasSearch || filter !== 'all') && (
        <button onClick={onClear} className="text-[12.5px] text-accent hover:underline">
          Clear filters
        </button>
      )}
    </div>
